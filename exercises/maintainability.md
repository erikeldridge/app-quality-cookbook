...
Code that is easier to read is easier to understand. Code that is easier to understand is easier to maintain. Code that is easier to maintain has less bugs. Code that has few bugs is less frightening to deploy. – @javi
...
The method should read configuration from a features.yml file:
...
Java refers to a file like this as a resource, and Maven defines a standard "resources" directory, eg /home/vagrant/my-app/src/main/resources/features.yml
...
Implement the function with minimal code.
...
We'll revisit this code, but we want to have relatively fresh eyes when we do. Take a break to clear your head, and then read through SourceMaking's description of spaghetti code.

Next, update your code to enable features based on os version. For example, given a config like this:
...
Functional programming lends itself to testing by preferring functions which take input, return output, and avoid side effects. Java isn't a functional language, strictly speaking, but we can still limit side effects. In our case, writing to standard out is a side effect.

Isolate this side effect by pulling the code that determines if a feature is enabled out of the main function. We can then test this code directly.
...
Maven's standard layout defines a src/test/resources directory we can use:
...
The practice of dependency injection refers to identifying things, like data sources, that our code depends on and providing them explicitly to our code. This becomes especially valuable as our dependencies become more costly to query, eg fetching from a remote data store.
...
We can split dependencies from the code using those dependencies without much fuss. Sometimes this is sufficient, however, several frameworks exist, eg Guice, Dagger 2, etc, to help organize the dependencies in our project.

Having identified dependencies, we’re now free to mock them in our tests. We’ll use Mockito as our mocking framework.
...
Note these tests are not dependent on loading a file in our test environment. We can have our mocks return any content that makes sense in a test, and verify the mock object was interacted with correctly.
...
However, programming languages are structured the way they are largely to be readable by the people who have to maintain code. Otherwise, we'd just write in binary.

So, let's modify our variable and function names to be more self-documenting.
...
A design pattern “is a general repeatable solution to a commonly occurring problem in software design” – SourceMaking.
...
The conditional logic in our selection method is difficult to read and modify. This is an example of a code smell, "a surface indication that usually corresponds to a deeper problem in the system" – Martin Fowler. Many code smells are associated with patterns which can be used to alleviate the smell.
...
The method must be static because we need to call it before we instantiate the FeatureSelector class.

The hashCode and equals functions enable us compare instances of the Inputs class, eg assertEquals(expected, actual). Use IntelliJ's generator to generate them: Code > Generate ... > equals() and hashCode()
...
Unix philosophy recommends we "do one thing and do it well".

Code that performs a single action is relatively easy to reason about and maintain.

To paraphrase the principles of separation of concerns and single-responsability, each piece of our code should perform a distinct role.
